...
namespace Bukimedia.PrestaSharp.Factories
{
    public class TaxRuleFactory : RestSharpFactory
    {
        ...
        public Task<Entities.tax_rule> Get(long TaxRuleId)
        {
            RestRequest request = this.RequestForGet("tax_rules", TaxRuleId, "tax_rule");
            return this.Execute<Entities.tax_rule>(request);
        }

        public async Task<Entities.tax_rule> Add(Entities.tax_rule TaxRule)
        {
            ...
            Entities.tax_rule aux = await this.Execute<Entities.tax_rule>(request);
            TaxRule.id = idAux;
            return await this.Get((long)aux.id);
        }

        public Task Update(Entities.tax_rule TaxRule)
        {
            RestRequest request = this.RequestForUpdate("tax_rules", TaxRule.id, TaxRule);
            return this.Execute<Entities.tax_rule>(request);
        }

        public Task Delete(long TaxRuleId)
        {
            RestRequest request = this.RequestForDelete("tax_rules", TaxRuleId);
            return this.Execute<Entities.tax_rule>(request);
        }

        public Task Delete(Entities.tax_rule TaxRule)
        {
            return this.Delete((long)TaxRule.id);
        }
        ...
        /// <returns></returns>
        public Task<List<Entities.tax_rule>> GetByFilter(Dictionary<string, string> Filter, string Sort, string Limit)
        {
        ...
        public async Task<List<long>> GetIdsByFilter(Dictionary<string, string> Filter, string Sort, string Limit)
        {
            ...
            List<PrestaSharp.Entities.FilterEntities.tax_rule> aux = await this.Execute<List<PrestaSharp.Entities.FilterEntities.tax_rule>>(request);
        ...
        public Task<List<Entities.tax_rule>> GetAll()
        {
        ...
        /// <returns></returns>
        public Task<List<Entities.tax_rule>> AddList(List<Entities.tax_rule> TaxRules)
        {
            List<PrestaSharp.Entities.PrestaShopEntity> Entities = new List<PrestaSharp.Entities.PrestaShopEntity>();
